...
public:
    ...
    bool calibrate(const CalibrationData& data, CalibrationMethod method = CalibrationMethod::POLYNOMIAL_FIT);
    
    // Position estimation
    PositionEstimate estimatePosition(double encoder_count, double velocity = 0.0) const;
    PositionEstimate estimatePositionWithUncertainty(double encoder_count, 
                                                   const Eigen::VectorXd& environmental_state) const;
...
public:
    ...
    bool calibrateEncoder(const std::string& encoder_id, const EncoderCalibration::CalibrationData& data);
