...
function SensorCard({ label, entity, component, unit = 'PSI', color, nop, meop }: SensorCardProps) {
  ...
  return (
    <div className={`bg-card border rounded px-1.5 py-1 hover:border-gray-600 transition-all ${statusClass}`}>
      <div className="text-[9px] text-text-muted font-semibold tracking-wider uppercase truncate mb-0.5">
        {label}
      </div>
      <div
      ...
      <div className="text-[8px] text-text-muted">{unit}</div>
    </div>
  );
}
...
function ActuatorPill({ label, entity }: { label: string; entity: string }) {
  ...
  return (
    <div className={`flex flex-col items-center justify-center bg-card border rounded px-1.5 py-1 gap-0.5 transition-all min-h-[48px] ${!hasData ? 'border-gray-800' : isOpen ? 'border-green-700/80' : 'border-red-700/80'
      }`}>
      <span className="text-[9px] font-bold text-text-muted uppercase tracking-wider text-center leading-tight">{label}</span>
      <span
        className={`text-xs font-bold font-mono px-1.5 py-1 rounded w-full text-center ${!hasData ? 'text-gray-600 bg-gray-900/40' : isOpen ? 'bg-green-900/50 text-green-300' : 'bg-red-900/50 text-red-300'
          }`}
      >
...
function SectionHeader({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-1.5 mb-0.5">
      ...
      <h2 className="text-[10px] font-bold tracking-widest text-text-muted uppercase">{children}</h2>
    </div>
  );
}
...
export default function Home() {
  ...
  }, [ws, updateSensor, updateState, updateConnectionStatus, updateMissionStartTime, updateBoards, updateNotification, updateActuator, updateActuatorExpectedPositions]);
  ...
  return (
    <main className="h-full min-h-0 flex flex-col overflow-hidden bg-background text-text">
      <div className="w-full flex-1 min-h-0 flex flex-col overflow-hidden px-2 py-1 gap-0.5">
        ...
        <div className="flex-shrink-0">
          ...
          <div className="grid grid-cols-5 lg:grid-cols-9 gap-0.5">
        ...
        <div className="flex-shrink-0">
          ...
          <div className="grid grid-cols-4 lg:grid-cols-7 gap-0.5">
            {actuators.map((a) => (
        ...
        {/* ── Windows (fills remaining space, scrolls internally if needed) ─ */}
