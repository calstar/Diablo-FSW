...
if [ ! -f "$CACHE" ]; then
    printf "  ${ERR} build/CMakeCache.txt not found — cmake has not been run\n"
...
check_node_env() {
    ...
    if [ ! -f "$lockfile" ]; then
        printf "    ${ERR} package-lock.json not found\n"
        error "$label: package-lock.json missing at $dir"
        return
    fi

    if [ ! -d "$modules" ]; then
        printf "    ${ERR} node_modules/ not found\n"
    ...
    if [ "$lock_mtime" -gt "$mod_mtime" ]; then
        ...
        printf "    ${OK} node_modules is up-to-date with package-lock.json\n"
    fi
...
if [ ! -d "$VENV" ]; then
    ...
    if [ -z "${SITE_PACKAGES:-}" ]; then
        printf "  ${ERR} Cannot locate site-packages in .venv\n"
        ...
        check_py_pkg() {
            ...
            norm="${norm//-/_}"
            local info
            info="$(ls -d "${SITE_PACKAGES}/${pkg}-"*.dist-info 2>/dev/null | head -1)"
            [ -z "$info" ] && info="$(ls -d "${SITE_PACKAGES}/${norm}-"*.dist-info 2>/dev/null | head -1)"
            local expected
...
if [ -z "$ELODIN_BIN" ]; then
    printf "  ${ERR} elodin-db not found (checked PATH and ~/.cargo/bin/)\n"
    ...
    if [ "$PLATFORM" = "WSL" ] && echo "$ELODIN_BIN" | grep -q "^/mnt/"; then
        printf "  ${WARN} elodin-db is on Windows filesystem — may have interop issues\n"
...
if [ ! -f "$CONFIG_TOML" ]; then
    printf "  ${ERR} config/config.toml NOT FOUND — system cannot start without it\n"
    error "config/config.toml missing"
else
    printf "  ${OK} config/config.toml exists\n"
...
if [ "$ANY_IN_USE" = true ]; then
    printf "\n  ${CYAN}Note:${NC} If ports show the correct services (elodin-db, node, next-server),\n"
...
if [ "${#WARNINGS[@]}" -eq 0 ]; then
    printf "\n  ${GREEN}${BOLD}ALL CHECKS PASSED — environment looks healthy.${NC}\n\n"
else
    printf "\n  Found %d issue(s):\n\n" "${#WARNINGS[@]}"
