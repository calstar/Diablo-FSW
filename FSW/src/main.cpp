...
bool initializeSubsystems() {
    ...
    try {
        ...
        initial_nav_state.covariance_matrix = Eigen::MatrixXd::Identity(EKFNavigation::STATE_DIM, EKFNavigation::STATE_DIM) * 0.01;
        ...
        scheduling_config.primary_variables = {
            GainScheduling::SchedulingVariable::CHAMBER_PRESSURE,
            GainScheduling::SchedulingVariable::THRUST
        };
        scheduling_config.secondary_variables = {
            GainScheduling::SchedulingVariable::MIXTURE_RATIO,
        ...
        if (!optimal_controller->initialize(OptimalController::ControlAlgorithm::MODEL_PREDICTIVE_CONTROL, 
        ...
        comm_config.telemetry.telemetry_types = {
            CommunicationProtocol::MessageType::ENGINE_STATUS,
        ...
        packet_protocol->registerSensorHandler(PacketProtocol::SensorType::PRESSURE_TRANSDUCER,
            [](const PacketProtocol::SensorData& sensor_data) {
                // Route pressure transducer data to navigation and control systems
                std::cout << "📊 Received pressure transducer data from sensor " << (int)sensor_data.sensor_id << std::endl;
        ...
        packet_protocol->registerSensorHandler(PacketProtocol::SensorType::IMU_ACCELEROMETER,
            [](const PacketProtocol::SensorData& sensor_data) {
                ...
                std::cout << "🧭 Received IMU accelerometer data" << std::endl;
                // TODO: Process IMU data and update navigation system
            }
        );
        ...
        packet_protocol->registerSensorHandler(PacketProtocol::SensorType::GPS_POSITION,
            [](const PacketProtocol::SensorData& sensor_data) {
                ...
                std::cout << "🛰️ Received GPS position data" << std::endl;
                // TODO: Process GPS data and update navigation system
            }
        );
        ...
        comm_protocol->registerMessageHandler(CommunicationProtocol::MessageType::ABORT_COMMAND,
            [](const CommunicationProtocol::Message& msg) {
