...
inline static postcard_error_t encode_unsigned_varint(postcard_slice_t* slice,
                                                      uint64_t value,
                                                      size_t max_bytes) {
    if (!slice || !slice->data) return POSTCARD_ERROR_INVALID_INPUT;

    size_t i = 0;
    while (value >= 0x80) {
        if (slice->len + i >= slice->capacity)
            return POSTCARD_ERROR_BUFFER_TOO_SMALL;
        if (i >= max_bytes) return POSTCARD_ERROR_OVERFLOW;
...
inline static postcard_error_t encode_signed_varint(postcard_slice_t* slice,
                                                    int64_t value,
                                                    size_t max_bytes) {
...
inline static postcard_error_t decode_unsigned_varint(postcard_slice_t* slice,
                                                      uint64_t* value,
                                                      size_t max_bytes) {
    if (!slice || !slice->data || !value) return POSTCARD_ERROR_INVALID_INPUT;
    ...
    *value = 0;
    ...
    while (i < max_bytes) {
        ...
        *value |= ((uint64_t)(byte & 0x7F)) << shift;
        if (!(byte & 0x80)) return POSTCARD_SUCCESS;

        shift += 7;
        if (shift > 63) return POSTCARD_ERROR_OVERFLOW;
    }
...
inline static postcard_error_t decode_signed_varint(postcard_slice_t* slice,
    ...
    if (err != POSTCARD_SUCCESS) return err;
...
inline postcard_error_t postcard_encode_bool(postcard_slice_t* slice,
                                             bool value) {
    if (!slice || !slice->data) return POSTCARD_ERROR_INVALID_INPUT;
    if (slice->len >= slice->capacity) return POSTCARD_ERROR_BUFFER_TOO_SMALL;
...
inline postcard_error_t postcard_decode_bool(postcard_slice_t* slice,
                                             bool* value) {
    if (!slice || !slice->data || !value) return POSTCARD_ERROR_INVALID_INPUT;
    if (slice->len >= slice->capacity) return POSTCARD_ERROR_INCOMPLETE_DATA;
...
inline postcard_error_t postcard_decode_u16(postcard_slice_t* slice,
    ...
    if (val > UINT16_MAX) return POSTCARD_ERROR_OVERFLOW;
    *value = (uint16_t)val;
    return POSTCARD_SUCCESS;
}
...
inline postcard_error_t postcard_decode_i16(postcard_slice_t* slice,
    ...
    if (val < INT16_MIN || val > INT16_MAX) return POSTCARD_ERROR_OVERFLOW;
    *value = (int16_t)val;
    return POSTCARD_SUCCESS;
}
...
inline postcard_error_t postcard_decode_u32(postcard_slice_t* slice,
    ...
    if (val > UINT32_MAX) return POSTCARD_ERROR_OVERFLOW;
    *value = (uint32_t)val;
    return POSTCARD_SUCCESS;
}
...
inline postcard_error_t postcard_decode_i32(postcard_slice_t* slice,
    ...
    if (val < INT32_MIN || val > INT32_MAX) return POSTCARD_ERROR_OVERFLOW;
    *value = (int32_t)val;
    return POSTCARD_SUCCESS;
}
...
inline postcard_error_t postcard_decode_f32(postcard_slice_t* slice,
                                            float* value) {
    if (!slice || !slice->data || !value) return POSTCARD_ERROR_INVALID_INPUT;
    if (slice->len + 4 > slice->capacity) return POSTCARD_ERROR_INCOMPLETE_DATA;
...
inline postcard_error_t postcard_decode_f64(postcard_slice_t* slice,
                                            double* value) {
    if (!slice || !slice->data || !value) return POSTCARD_ERROR_INVALID_INPUT;
    if (slice->len + 8 > slice->capacity) return POSTCARD_ERROR_INCOMPLETE_DATA;
...
inline postcard_error_t postcard_decode_byte_array_len(postcard_slice_t* slice,
    ...
    if (len > SIZE_MAX) return POSTCARD_ERROR_OVERFLOW;
...
inline postcard_error_t postcard_decode_byte_array(postcard_slice_t* slice,
    ...
    if (actual_length > max_length) return POSTCARD_ERROR_BUFFER_TOO_SMALL;
...
inline postcard_error_t postcard_decode_option_tag(postcard_slice_t* slice,
                                                   bool* is_some) {
    if (!slice || !slice->data || !is_some) return POSTCARD_ERROR_INVALID_INPUT;
    if (slice->len >= slice->capacity) return POSTCARD_ERROR_INCOMPLETE_DATA;
...
class InitialTimestamp
    : public std::variant<std::monostate, std::monostate, int64_t> {
   public:
    ...
    bool is_earliest() const { return this->index() == 0; }
    ...
    bool is_latest() const { return this->index() == 1; }
    ...
    bool is_manual() const { return this->index() == 2; }
    ...
    size_t encoded_size() const {
        ...
        if ([[maybe_unused]] auto val = std::get_if<0>(
                (const std::variant<std::monostate, std::monostate,
                                    int64_t>*)this)) {
    ...
    postcard_error_t decode(std::span<const uint8_t>& input) {
        postcard_slice_t slice;
        postcard_init_slice(&slice, const_cast<uint8_t*>(input.data()),
                            input.size());
...
class StreamBehavior : public std::variant<std::monostate, FixedRateBehavior> {
   public:
    ...
    bool is_real_time() const { return this->index() == 0; }
    ...
    std::monostate* get_real_time() {
        return std::get_if<0>(
            (std::variant<std::monostate, FixedRateBehavior>*)this);
    }
    ...
    bool is_fixed_rate() const { return this->index() == 1; }
...
class Op : public std::variant<OpData, OpTable, std::monostate, OpPair,
                               OpSchema, OpTimestamp, OpExt> {
   public:
    static constexpr std::string_view TYPE_NAME = "Op";
    // Inherit constructors from std::variant
    using std::variant<OpData, OpTable, std::monostate, OpPair, OpSchema,
                       OpTimestamp, OpExt>::variant;
    ...
    bool is_data() const { return this->index() == 0; }

    const OpData* get_data() const {
        return std::get_if<0>(
            (const std::variant<OpData, OpTable, std::monostate, OpPair,
                                OpSchema, OpTimestamp, OpExt>*)this);
    }

    OpData* get_data() {
        return std::get_if<0>(
            (std::variant<OpData, OpTable, std::monostate, OpPair, OpSchema,
    ...
    bool is_table() const { return this->index() == 1; }
    ...
    static Op None() { return Op{std::in_place_index<2>, std::monostate{}}; }

    // Accessor method for variant None
    bool is_none() const { return this->index() == 2; }
    ...
    bool is_pair() const { return this->index() == 3; }
    ...
    bool is_schema() const { return this->index() == 4; }
    ...
    bool is_timestamp() const { return this->index() == 5; }
    ...
    bool is_ext() const { return this->index() == 6; }
    ...
    size_t encoded_size() const {
        ...
        if ([[maybe_unused]] auto val = std::get_if<0>(
            ...
            size += (*val).encoded_size();
                       ...
                       (const std::variant<OpData, OpTable, std::monostate,
                                           OpPair, OpSchema, OpTimestamp,
                                           OpExt>*)this)) {
            size += (*val).encoded_size();
        } else if ([[maybe_unused]] auto val = std::get_if<2>(
                       (const std::variant<OpData, OpTable, std::monostate,
    ...
    postcard_error_t encode_raw(postcard_slice_t* slice) const {
        ...
        if ([[maybe_unused]] auto val = std::get_if<0>(
            ...
            if (result != POSTCARD_SUCCESS) return result;
                       ...
                       (std::variant<OpData, OpTable, std::monostate, OpPair,
                                     OpSchema, OpTimestamp, OpExt>*)this)) {
            result = postcard_encode_u8(slice, 1);
            if (result != POSTCARD_SUCCESS) return result;
            result = (*val).encode_raw(slice);
...
struct OpRef {
    ...
    postcard_error_t encode(std::span<std::byte>& output) const {
        postcard_slice_t slice;
        postcard_init_slice(&slice, reinterpret_cast<uint8_t*>(output.data()),
                            output.size());
        auto res = encode_raw(&slice);
        if (res != POSTCARD_SUCCESS) return res;
        output = output.subspan(0, slice.len);
    ...
    std::vector<std::byte> encode_vec() const {
        ...
        // Encode into the span
        postcard_slice_t slice;
        postcard_init_slice(&slice, reinterpret_cast<uint8_t*>(span.data()),
                            span.size());
        auto res = encode_raw(&slice);
...
class PrimType
    : public std::variant<std::monostate, std::monostate, std::monostate,
   ...
   public:
    ...
    // Inherit constructors from std::variant
    using std::variant<std::monostate, std::monostate, std::monostate,
                       std::monostate, std::monostate, std::monostate,
                       std::monostate, std::monostate, std::monostate,
                       std::monostate, std::monostate>::variant;
    ...
    bool is_u_8() const { return this->index() == 0; }
    ...
    std::monostate* get_u_8() {
        return std::get_if<0>(
            (std::variant<std::monostate, std::monostate, std::monostate,
                          std::monostate, std::monostate, std::monostate,
                          std::monostate, std::monostate, std::monostate,
                          std::monostate, std::monostate>*)this);
    }
    ...
    bool is_u_16() const { return this->index() == 1; }
    ...
    bool is_u_32() const { return this->index() == 2; }
    ...
    bool is_u_64() const { return this->index() == 3; }
    ...
    bool is_i_8() const { return this->index() == 4; }
    ...
    bool is_i_16() const { return this->index() == 5; }
    ...
    bool is_i_32() const { return this->index() == 6; }
    ...
    bool is_i_64() const { return this->index() == 7; }
    ...
    bool is_bool() const { return this->index() == 8; }
    ...
    bool is_f_32() const { return this->index() == 9; }
    ...
    bool is_f_64() const { return this->index() == 10; }
    ...
    size_t encoded_size() const {
        ...
        if ([[maybe_unused]] auto val =
                std::get_if<0>((const std::variant<
                                std::monostate, std::monostate, std::monostate,
                                std::monostate, std::monostate, std::monostate,
                                std::monostate, std::monostate, std::monostate,
                                std::monostate, std::monostate>*)this)) {
...
struct ComponentMetadata {
    ...
    postcard_error_t decode_raw(postcard_slice_t* slice) {
        ...
        if (name_len > 0) {
            result =
                postcard_decode_string(slice, name.data(), name_len, name_len);
            if (result != POSTCARD_SUCCESS) return result;
        }
        if (result != POSTCARD_SUCCESS) return result;
...
inline std::shared_ptr<OpBuilder> data(const std::vector<uint8_t>& bytes,
                                       size_t align = 1) {
...
inline std::shared_ptr<OpBuilder> pair(uint64_t entity_id,
...
/// argument
inline std::shared_ptr<OpBuilder> timestamp(
    std::shared_ptr<OpBuilder> timestamp, std::shared_ptr<OpBuilder> arg) {
...
inline FieldBuilder raw_field(uint16_t offset, uint16_t len,
                              std::shared_ptr<OpBuilder> arg) {
...
inline FieldBuilder field(std::shared_ptr<OpBuilder> arg) {
    ...
    size_t offset =
        reinterpret_cast<size_t>(&(reinterpret_cast<Class*>(0)->*MemberPtr));
    size_t size = sizeof(FieldType);
    return raw_field(static_cast<uint16_t>(offset), static_cast<uint16_t>(size),
                     std::move(arg));
}
...
class VTableBuilder {
   ...
   public:
            ...
            [&](const auto& val) {
                ...
                if constexpr (std::is_same_v<T, OpBuilder::Data>) {
                    ...
                    const size_t padding =
                        (align - (vtable.data.size() % align)) % align;
                    ...
                    const uint16_t offset =
                        static_cast<uint16_t>(vtable.data.size());
                    const uint16_t len =
                        static_cast<uint16_t>(data_op.data.size());

                    vtable.data.insert(vtable.data.end(), data_op.data.begin(),
                                       data_op.data.end());
                    ...
                    result_op =
                        Op::Table(OpTable{table_op.offset, table_op.len});
                    ...
                    result_op = Op::Pair(
                        OpPair{static_cast<uint16_t>(entity_id.value),
                    ...
                    result_op =
                        Op::Schema(OpSchema{static_cast<uint16_t>(ty.value),
                    ...
                    result_op = Op::Timestamp(
                        OpTimestamp{static_cast<uint16_t>(timestamp.value),
                                    static_cast<uint16_t>(arg.value)});
                    ...
                    result_op = Op::Ext(
                        OpExt{static_cast<uint16_t>(arg.value), ext_op.id,
    ...
    void push_field(const FieldBuilder& field_builder) {
        ...
        Field field{field_builder.offset, field_builder.len,
                    static_cast<uint16_t>(arg.value)};
    ...
    VTable build() const { return vtable; }
};
